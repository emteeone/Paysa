...
using UGB.Paysa.Identity;
using UGB.Paysa.Schemas;
using UGB.Paysa.Web.Chat.SignalR;
using UGB.Paysa.Web.Common;
using UGB.Paysa.Web.Resources;
using Swashbuckle.AspNetCore.Swagger;
using UGB.Paysa.Web.IdentityServer;
using UGB.Paysa.Web.Swagger;
using Stripe;
...
using UGB.Paysa.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using Microsoft.Extensions.DependencyInjection.Extensions;
using UGB.Paysa.Web.Extensions;
using SecurityStampValidatorCallback = UGB.Paysa.Identity.SecurityStampValidatorCallback;

namespace UGB.Paysa.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<PaysaWebMvcModule>(options =>
            {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "Paysa API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("UGB.Paysa.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            services.AddSwaggerGen(options =>
            {
                ...
                var applicationXml = $"UGB.Paysa.Application.xml";
                var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                options.IncludeXmlComments(applicationXmlPath);

                var webCoreXmlFile = $"UGB.Paysa.Web.Core.xml";
